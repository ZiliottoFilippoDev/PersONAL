...
from habitat.tasks.nav.personalized_object_nav_task import (
    ObjectGoal,
...
@registry.register_dataset(name="Personalized-ObjectNav-v1")
class PersonalizedObjectNavDatasetV1(PointNavDatasetV1):
    ...
    def from_json(
        ...
        for k, v in deserialized["goals_by_category"].items():
            self.goals_by_category[k] = [self._deserialize_goal(g) for g in v]
        ...
        for i, episode in enumerate(deserialized["episodes"]):
            ...
            # Delete different floor goals and object_ids
            episode.goals = [
                g for g in episode.goals if g.floor_id in episode.floor_id
                and g.object_name in episode.object_id
            ]
