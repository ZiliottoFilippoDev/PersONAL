...
class ZSONPolicyNet(Net):
    def __init__(
        ...
        self.emb_cache = {}
        print(f"Multi Query : {multi_query}, Scrape num: {scrape_num}")
...
## : SCRAPE FOR IMAGE PROMPTS

def load_images(query, 
                img_dir = "/mnt/vlfm_query_embed/data/scraped_imgs/ovon_15", 
                scrape_num = 15):

    q_dir = os.path.join(img_dir, query)
    assert os.path.exists(q_dir)

    q_img_paths = [os.path.join(q_dir, p) for p in os.listdir(q_dir)]
    q_img_paths = sorted(q_img_paths, 
                        key=lambda p : int( p.split("/")[-1].split(".")[0] ), 
                        reverse=False)
    q_img_paths = q_img_paths[:scrape_num]
    assert len(q_img_paths) == scrape_num

    print(f"Loaded {scrape_num} images for {query}.")

    imgs = []
    for p in q_img_paths:
        img = transforms.PILToTensor()(Image.open(p).convert("RGB"))
...
def get_embeds(imgs, model, model_transform):

    img_embeds = []

    for img in imgs:
        with torch.no_grad():
            emb_img = model_transform(img, None, None, None).to(device="cuda")
            emb_img = model.encode_image(emb_img).float()
            emb_img /= emb_img.norm(dim=-1, keepdim=True)

        img_embeds.append(emb_img)
